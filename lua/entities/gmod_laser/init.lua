...
function ENT:Think()
  if ( self:GetOn() ) then
    ...
    repeat
      ...
      if ( trace.Entity and trace.Entity:IsValid() and trace.Entity:GetModel() == gsReflector ) then
        isMirror = true
    ...
    until ( isMirror == false or bounces > varMaxBounces:GetInt() )

    if( self:GetDamageAmmount() > 0 and trace.Entity and trace.Entity:IsValid() and
      trace.Entity:GetClass() ~= "gmod_laser" and trace.Entity:GetModel() ~= gsReflector ) then
