...
DATA.MXFRESNE = CreateConVar(DATA.TOOL.."_mxfresnel" , 2    , DATA.FGSRVCN, "Maximum fresnel effect medium traverse splits", 0, 10)
...
DATA.WDHUECNT = CreateConVar(DATA.TOOL.."_wdhuecnt"  , 10   , DATA.FGSRVCN, "Hue count when using dispersion and splitting color components", 0, 50)
...
function LaserLib.DrawPoint(pos, mar, col, idx, msg)
  if(SERVER) then return end
  local crw = LaserLib.GetColor(col or "YELLOW")
  render.SetColorMaterial()
  render.DrawSphere(pos, mar / 10, 25, 25, crw)
  if(idx or msg) then
...
function LaserLib.DrawVector(pos, dir, mag, col, idx, msg)
  ...
  local crw = LaserLib.GetColor(col or "YELLOW")
  render.SetColorMaterial()
  render.DrawSphere(pos, mag / 10, 25, 25, crw)
...
function mtBeam:SetFgTexture(bNov, bDsp, bFre)
  if(bNov ~= nil) then self.BmNoover = tobool(bNov) end -- No override material flag. Try to extract original material
  if(bDsp ~= nil) then self.BmDisper = tobool(bDsp) end -- Dispersion enable flag. Split white light to its components
  if(bFre ~= nil) then self.BmFresne = tobool(bFre) end -- Fresnel effect interface enable flag. Reflect from medium interface
  return self
end
...
function mtBeam:SetFgDivert(bRfl, bRfr)
  if(bRfl ~= nil) then self.BrReflec = tobool(bRfl) end -- Reflection ratio flag. Reduce beam power when reflecting
  if(bRfr ~= nil) then self.BrRefrac = tobool(bRfr) end -- Refraction ratio flag. Reduce beam power when refracting
  return self
end
...
function mtBeam:GetWaterOrigin(pos)
  if(self:IsAir()) then return nil end
  local wat = self:GetWater()
  local org = Vector(pos or self.VrOrigin)
  org:Sub(wat.P); return org:Dot(wat.N)
end
...
function mtBeam:SetWaterSurface(origin, direct, length, filter)
  ...
  local dir = Vector(direct or DATA.VDRUP)
  local org = Vector(origin or self.VrOrigin)
...
--[[
 * Moves the start recursive beam origin away
 * so that the recursive beam can be run properly
 * vO > Base origin beam start location
 * vD > Base direction beam start location
 * nM > Margin amount to move along the ray
]]
...
function mtBeam:SetMedium(index, data, mkey, norm, opos)
  if(not index) then return self end
  local merum = self.TrMedium -- Localize beam mediums
  local info = merum[index] -- Index the desired medium by key
  if(info) then -- Entry is present for this cached medium
    info[1], info[2] = (data or info[1]), (mkey or info[2])
    info[3], info[4] = (norm or info[3]), (opos or info[4])
  else -- Entry for this cached medium is unavailable
    local meair = mtBeam.__meair; merum[index] = {}
    local info = merum[index] -- Index the desired medium by key
    info[1], info[2] = (data or meair[1]), (mkey or meair[2])
    info[3], info[4] = (norm or Vector()), (opos or Vector())
  end; return self
end
...
function mtBeam:SetMediumName(name, medium, key, normal, origin)
  ...
  if(normal) then info[3]:Set(normal) end
  if(origin) then info[4]:Set(origin) end
  return self -- Coding effective API
end
--[[
...
function mtBeam:SetTraceWidth(trace, length)
  if(trace and  -- Check if the trace is available
    ...
    local nor = Vector(trace.HitNormal) -- Normal scale
    nor:Mul(-DATA.TRDGQ * self.BmTracew)trace.HitPos:Add(nor)
...
function mtBeam:RegisterNode(origin, nbulen, bedraw)
  ...
  if(cnlen > 0) then -- Subtract the path trough the medium
    ...
    if(size > 0 and nbulen) then -- Length is not provided
      ...
      local vdir = Vector(node); vdir:Sub(prev)
      self:Pass(vdir:Length()) -- Slower but correct
...
function mtBeam:SetTraceExit()
  ...
  local org = Vector(self.VrOrigin)
  local dir = Vector(self.VrDirect); dir:Mul(len)
...
function mtBeam:RefractWaterAir()
  ...
  self:RegisterNode(vwa, true)
  local vdir, bnex = self:Refract(vwa, dir, wat.N, mewat[1][1], meair[1][1])
  if(bnex) then
...
function mtBeam:SetRefractWorld(refract, key)
  ...
  if(self:IsAir()) then -- There is no water surface registered
    ...
    merum.S[1], merum.D[1] = merum.D[1], merum.S[1] -- Refractive index
    merum.S[2], merum.D[2] = merum.D[2], merum.S[2] -- Reduction ratio
  end
end
...
function mtBeam:IsTraverse(origin, direct, normal, target)
  local org = Vector(origin or self.VrOrigin)
  local dir = Vector(direct or self.VrDirect)
  ...
  if(bnex) then
    ...
    if(self.BrRefrac and not bsam) then self:SetPowerRatio(merum.D[1][2]) end
  ...
  if(self.StRfract) then -- Bounces were decremented so move it up
    if(self:IsFirst()) then
      ...
      if(self:IsMemory(index)) then
        vdir, bnex, bsam = self:Refract(org, dir, merum.M[3], merum.M[1][1], index)
    ...
    vdir, bnex, bsam = self:Refract(org, dir, trace.HitNormal, merum.S[1][1], index)
  end
...
DATA.ACTORS = {
  ...
  ["gmod_laser_refractor"] = function(beam)
    ...
    local ovr, dis, frn = beam:GetFgTexture()
    ...
    end; if(not dis) then beam:Finish(false) end
     -- Apply refraction ratio
 ...
 * Checks whenever dispersion is enabled and try to decompose
 * Or the beam is monochromatic and should not be branched
 * vOrg  > Origin start for splitting the beam
 * vDir  > Direction for splitting source entities
 * nSrc  > Source medium refractive index
 * nDst  > Destination medium refractive index
 * [...] > The results from the library refraction
]]
function mtBeam:Disperse(vOrg, vDir, vNor, nSrc, nDst, bNex, bSam, nCos)
  -- Dispersion is not being enabled
  if(not self.BmDisper) then return end
  ...
  if(self.StRfract and self:IsFirst()) then return end
  -- Check if we have more bounces
  local bnc = self:GetBounces(true)
  if(bnc <= 0) then return end
  -- Equal refractive indices then no dispersion
  if(nSrc == nDst or bSam) then return end
...
function mtBeam:Fresnel(vOrg, vDir, vNor, nSrc, nDst, bNex, bSam, nCos)
  -- Beam start inside a solid medium
  if(self.StRfract) then return end
 ...
 * Returns [vdir, bnex, bsam, ncos] according to wavelength
]]
function mtBeam:Refract(vOrg, vDir, vNor, nSrc, nDst)
  ...
  local vOrg = Vector(vOrg or tTg.HitPos or self.VrOrigin)
  ...
  self:Fresnel(vOrg, vDir, vNor, nSrc, nDst, bNex, bSam, nCos)
  self:Disperse(vOrg, vDir, vNor, nSrc, nDst, bNex, bSam, nCos)
    ...
    if(self:IsFirst()) then -- Initial start so the beam separates from the laser
      ...
      if(trace.StartSolid) then -- The beam starts in solid map environment
        if(not self:SetRefractContent(trace.Contents, trace)) then -- Switch medium via trace content
          self:SetRefractContent(util.PointContents(self.VrOrigin), trace) -- Trace contents not matched
        ...
        if(self.IsRfract) then
          ...
          if(self.NvLength > 0) then
            if(not self:IsTraverse(trace.HitPos, nil, trace.HitNormal, target)) then
              -- Refract the hell out of this requested beam with entity destination
              local vdir, bnex = self:Refract(trace.HitPos, self.VrDirect,
          ...
          if(not trace.AllSolid) then
            ...
            if(self.NvLength > 0) then
              if(not self:IsTraverse(org, nil, nrm, target)) then
                local vdir, bnex = self:Refract(org, self.VrDirect,
